...
(function() {
    ...
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + path + '/basejs/resources/css/ext-all' + suffix + '-debug.css"/>');
    }
    document.write('<script type="text/javascript" src="' + path + '/basejs/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? path + '/..' : path) +
            '/basejs/packages/ext-theme-neptune/build/ext-theme-neptune' +
            (repoDevMode ? '-dev' : '') + '.js';
