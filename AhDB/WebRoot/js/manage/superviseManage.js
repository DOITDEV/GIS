...
//			}
			...
			superviseStore.setProxy({
				type: 'ajax',  
		        url: 'supervise/getSuperByHos.html?hosName='+encodeURI(Ext.getCmp('sup_HosValue').getValue()),
		        reader: {  
		            //数据格式为json  
		            type: 'json',  
		            root: 'supervise',  
...
//	}
	,{
		...
		handler: function (){
				...
				var startDate = Ext.getCmp('sup_startDate').getValue();
				var endDate = Ext.getCmp('sup_endDate').getValue();
				
				var startDate = Ext.Date.format(new Date(startDate),'Y-m-d'); 
				var endDate = Ext.Date.format(new Date(endDate),'Y-m-d'); 
				//alert(startDate);
				superviseStore.setProxy({
					type: 'ajax',  
			        url: 'supervise/getSuperByDate.html?begin='+startDate+"&end="+endDate,
			        reader: {  
			            //数据格式为json  
			            type: 'json',  
			            root: 'supervise',  
