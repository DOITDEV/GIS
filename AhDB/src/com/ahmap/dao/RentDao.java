...
public class RentDao {
		...
		public void addRent(RentInfo rent){
			...
			Object[] params = new Object[]{rent.getId(),rent.getLanBlock(),rent.getCityArea(),rent.getAddress(),rent.getPropertys(),rent.getPropertyNo(),rent.getLandSize(),rent.getRoomSize(),rent.getNonOcc(),rent.getGeoLocation(),
							rent.getRealDisplay(),rent.getIsRent(),rent.getCoors_x(),rent.getCoors_y(),rent.getFiller1(),rent.getFiller2(),rent.getFiller3()};
			jdbcTemplate.update(sql, params);
			
		}
		public void deleteRent(RentInfo rent){
			String sql="DELETE FROM rentinfo where id='"+rent.getId()+"'";
			jdbcTemplate.equals(sql);
			
		}
		//修改出租房信息
		public void updateRent(RentInfo rent){
			String sql="UPDATE rentmentinfo set lanblock=?,cityarea=?,address=?,propertys=?,propertyno=?,landsize=?,roomsize=?,nonocc=?,geolocation=?,realdisplay=?,isrent=?,coors_x=?,coors_y=?  where id=? and address=?";
			Object[] params = new Object[]{rent.getLanBlock(),rent.getCityArea(),rent.getAddress(),rent.getPropertys(),rent.getPropertyNo(),rent.getLandSize(),rent.getRoomSize(),rent.getNonOcc(),rent.getGeoLocation(),
					rent.getRealDisplay(),rent.getIsRent(),rent.getCoors_x(),rent.getCoors_y(),rent.getId(),rent.getAddress()};
			jdbcTemplate.update(sql, params);
		}
		public int getCount(){
			String sql="SELECT COUNT(*) FROM rentmentinfo";
			return jdbcTemplate.queryForInt(sql);
		}
		//根据城区统计数量
		public int getCountByArea(String cityarea){
			String sql="SELECT COUNT(*) FROM rentmentinfo where cityarea='"+cityarea+"'";
			return jdbcTemplate.queryForInt(sql);
		}
		
		public List<RentInfo> getAllRents(String start,String limit){
			String sqlStr = "SELECT * from rentmentinfo limit "+limit+" offset "+start;
		...
		public List<RentInfo> getAllRentByArea(String cityArea,String start,String limit){
			String sqlStr = "SELECT * from rentmentinfo  where  cityarea like '%"+ cityArea +"%'"+limit+" offset "+start;
			return jdbcTemplate.query(sqlStr, new RentMapper());
		}
		//根据地块查询
		public List<RentInfo> getAllRentsByBlock(String landBlock,String start,String limit){
			String sqlStr = "SELECT * from rentmentinfo where  landblock like '%"+ landBlock +"%'"+limit+" offset "+start;
			return jdbcTemplate.query(sqlStr, new RentMapper());
		}
		private static final class RentMapper implements RowMapper<RentInfo> {
			...
			public RentInfo mapRow(ResultSet rs, int rowNum)
				...
				rent.setIsRent(rs.getString("isrent"));
